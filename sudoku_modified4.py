...
class Grid:
	...
	def create_solving_state(self):
		...
		return solving_state			

	def sudoku_with_one_solution(self):
		self.random_generate_grid()
		solution = perform_filtering(self.create_solving_state(), get_standard_restrictions())
		choose_coordinate = [(i, j) for i in range(self.n * self.n) for j in range(self.n * self.n)]
		while any(len(solution[(i, j)]) == 1 for i in range(9) for j in range(9)):
			count_zeros = 0
			coordinate = random.choice(choose_coordinate)
			choose_coordinate.remove(coordinate)
			self.table[coordinate[0]][coordinate[1]] = 0
			solution = perform_filtering(self.create_solving_state(), get_standard_restrictions())
			for i in range(len(self.table)):
				count_zeros += self.table[i].count(0)
			if count_zeros >= 40:
				break				
